...
class Form{
  ...
  public function surround($name, $type){
    if( $type == 'submit' OR $type == 'reset'){
      return '<div>'.$this->submitBtn($name).'</div>';
    }
    else{
      return '<div>'.$this->input($name, $type).'</div>';
    }
  }
...
/**
  ...
  public function input($name, $type){
    return '<input type="'.$type.'" name="'.$name.'" placeholder="'.$name.'" id="'.$name.'" value="'.$this->getValue($name).'">';
  }
   ...
   * @return String       : balise <input>
   */
  public function submitBtn($name){
